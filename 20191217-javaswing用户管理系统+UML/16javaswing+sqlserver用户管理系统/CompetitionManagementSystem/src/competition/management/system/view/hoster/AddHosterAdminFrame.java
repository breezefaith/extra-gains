/*
 * Created by JFormDesigner on Sat Dec 21 13:56:23 CST 2019
 */
...
public class AddHosterAdminFrame extends JFrame {
    ...
    private void button_addActionPerformed(ActionEvent e) {
        ResourceBundle bundle = ResourceBundle.getBundle("competition.management.system.view.hoster.AddHosterAdminFrame");
        HosterAdmin hosterAdmin = new HosterAdmin();
        hosterAdmin.setId(textField_id.getText());
        hosterAdmin.setCode(textField_code.getText());
        hosterAdmin.setName(textField_name.getText());
        hosterAdmin.setSchool(textField_school.getText());
        hosterAdmin.setPassword(String.valueOf(passwordField.getPassword()));

        try {
            if (hosterAdminDao.insert(hosterAdmin)) {
                JOptionPane.showMessageDialog(this, bundle.getString("AddHosterAdminFrame.alert.add.success"), "", JOptionPane.INFORMATION_MESSAGE);
                hosterMainFrame.load();
                setVisible(false);
            } else {
                JOptionPane.showMessageDialog(this, bundle.getString("AddHosterAdminFrame.alert.add.error"), "", JOptionPane.ERROR_MESSAGE);
            }
        } catch (SQLException ex) {
            ex.printStackTrace();
            JOptionPane.showMessageDialog(this, bundle.getString("AddHosterAdminFrame.alert.add.error") + "\n" + ex.getMessage(), "", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void initComponents() {
        ...
        //======== this ========
        setTitle(bundle.getString("AddHosterAdminFrame.this.title"));
        Container contentPane = getContentPane();
        contentPane.setLayout(null);

        //---- label1 ----
        label1.setText(bundle.getString("AddHosterAdminFrame.label1.text"));
        contentPane.add(label1);
        label1.setBounds(new Rectangle(new Point(75, 55), label1.getPreferredSize()));

        //---- label2 ----
        label2.setText(bundle.getString("AddHosterAdminFrame.label2.text"));
        contentPane.add(label2);
        label2.setBounds(new Rectangle(new Point(65, 90), label2.getPreferredSize()));

        //---- label3 ----
        label3.setText(bundle.getString("AddHosterAdminFrame.label3.text"));
        contentPane.add(label3);
        label3.setBounds(new Rectangle(new Point(65, 130), label3.getPreferredSize()));

        //---- label4 ----
        label4.setText(bundle.getString("AddHosterAdminFrame.label4.text"));
        contentPane.add(label4);
        label4.setBounds(new Rectangle(new Point(65, 170), label4.getPreferredSize()));

        //---- label5 ----
        label5.setText(bundle.getString("AddHosterAdminFrame.label5.text"));
        contentPane.add(label5);
        label5.setBounds(new Rectangle(new Point(65, 215), label5.getPreferredSize()));

        //---- button_add ----
        button_add.setText(bundle.getString("AddHosterAdminFrame.button_add.text"));
