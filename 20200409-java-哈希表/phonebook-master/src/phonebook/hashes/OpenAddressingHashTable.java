...
public abstract class OpenAddressingHashTable implements HashTable {
    ...
    /* *************************************************************** */
    protected double loadFactor = 0.5d;
    protected int tombstoneCount = 0;
    ...
    protected int resize(int size) {
        ...
        for (int j = 0; j < oldCap; ++j) {
            ...
            if (e != null && !e.equals(this.TOMBSTONE)) {
                probes = probes + put(e.getKey(), e.getValue()).getProbes();
            }
        }
        return probes;
    }
    ...
    protected int resize(){
        return resize(primeGenerator.getNextPrime());
    }
}
