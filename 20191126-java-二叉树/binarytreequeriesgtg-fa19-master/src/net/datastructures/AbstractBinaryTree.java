...
public abstract class AbstractBinaryTree<E> extends AbstractTree<E>
  ...
  public List<E> suchThatIterative(BiFunction<BinaryTree<E>, Position<E>, Boolean> p) {
    ...
    List<Position<E>> nodes = new LinkedList<>();
    nodes.add(root());
    while (nodes.isEmpty() == false){
      Position<E> node = nodes.remove(nodes.size() - 1);
      if(p.apply(this, node) == true){
        res.add(node.getElement());
      }
      Position<E> left = left(node);
      Position<E> right = right(node);
      if(right(node) != null){
        nodes.add(right);
      }
      if(left(node) != null){
