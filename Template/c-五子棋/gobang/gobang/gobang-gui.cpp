...
int Check()//胜负检查，即判断当前走子位置有没有造成五连珠的情况
{
	...
	for (i = 1; i < 5; i++)if (Cx - i > 0 && Cy + i < MAXIMUS&&p[Cx - i][Cy + i] == Now)z++; else break;//向左下检查 if(z>=5)return Now;//若果达到5个则判断当前走子玩家为赢家
...
int main()//主函数
{
	system("title 五-子-棋 -- c语言(dotcpp.com)研究中心");//设置标题
