...
mage::mage(int lv_in, std::string name_in)
{
	...
	HP = HPmax;
	MPmax = 100 + 10 * (LV - 1);		// MP increases 2 points per level
	MP = MPmax;
	AP = 30 + 6 * (LV - 1);			// AP increases 4 points per level
	DP = 20 + 3 * (LV - 1);			// DP increases 4 points per level
	speed = 20 + 2 * (LV - 1);		// speed increases 2 points per level
...
void mage::isLevelUp()
{
	if (EXP >= LV * LV * 75)
	{
		...
		cout << "MP improved 10 points to " << MPmax << endl;
