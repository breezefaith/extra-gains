...
//broadcast to all clients except this one
                        ...
                        for (String clientName : writers.keySet()) {
                            if (clientName.equals(clientNames.get(currentNum)) == false) {
                                writers.get(clientName).println("Message from " + clientNames.get(currentNum) + ": " + received);
                            }
                        }
...
//TO-DO: Rest of the code similar to the basic multi-client/server program
            }
        }
    }
}